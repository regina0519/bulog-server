...
$val="";
foreach($objs as $obj){
    $obj["password"]=password_hash($obj["id_pegawai"], PASSWORD_DEFAULT);
    if($val==""){
        $val="(
            '".$obj["id_pegawai"]."',
            '".$obj["id_jab"]."',
...
$sql="";
if($val!=="")$sql="INSERT INTO tbl_pegawai(id_pegawai,id_jab,nm_pegawai,password,tipe_user,ganti_pass,aktif) VALUES".$val;

if ($conn->query($sql) === TRUE) {
    $msg = array('succeed' => '1', 'error' => '');
} else {
    if($conn->errno==1062){
        $msg = array('succeed' => '0', 'error' => "Maaf, ID Pegawai sudah terpakai.\nGagal menyimpan data.");
    }else{
